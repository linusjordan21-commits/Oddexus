#!/usr/bin/env node
/**
 * Unibet/Kambi prematch football scraper.
 *
 * Backend: https://eu-offering-api.kambicdn.com/offering/v2018/ubse/listView/
 ...
 *
 * Output: data/kambi-rows.json med events[]-shape.
 */
...
const OFFERING = "ubse"; // Unibet Sverige
const REFERER = "https://www.unibet.se/betting/sports/home";
...
const OUTPUT_FILE = path.join(DATA_DIR, "kambi-rows.json");
...
const KAMBI_DETAIL_MAX = Number(process.env.KAMBI_DETAIL_MAX) || 400;
...
async function main() {
  ...
  const deadline = installHardDeadline({
    budgetMs: Number(process.env.KAMBI_DEADLINE_MS) || 4 * 60 * 1000,
    label: "kambi",
  });
