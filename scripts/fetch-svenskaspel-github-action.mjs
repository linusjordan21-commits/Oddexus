#!/usr/bin/env node
/**
 * Svenska Spel / Oddset (Kambi-plattform) prematch football scraper.
 *
 * Oddset drivs av Kambi — IDENTISK feed-struktur som Unibet/ATG-scrapern, bara en
 * annan offering-kod ("svenskaspel"). Bekräftat via recon 2026-06-26 (spela.svensk
 * aspel.se/oddset anropar eu.offering-api.kambicdn.com/offering/v2018/svenskaspel).
 *
 * Backend: https://eu-offering-api.kambicdn.com/offering/v2018/svenskaspel/listView/
...
const OFFERING = "svenskaspel"; // Svenska Spel/Oddset (Kambi-offering, bekräftad via recon 2026-06-26)
const REFERER = "https://spela.svenskaspel.se/oddset";
...
const OUTPUT_FILE = path.join(DATA_DIR, "svenskaspel-rows.json");
...
const KAMBI_DETAIL_MAX = Number(process.env.SVENSKASPEL_DETAIL_MAX) || 400;
...
async function main() {
  ...
  const deadline = installHardDeadline({
    budgetMs: Number(process.env.SVENSKASPEL_DEADLINE_MS) || 4 * 60 * 1000,
    label: "svenskaspel",
  });
  ...
  const payload = {
    ...
    displayName: "Svenska Spel (Oddset)",
    group: "kambi",
    offering: OFFERING,
    status,
